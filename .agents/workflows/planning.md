...
1. Read all `context/` docs (design_system.md is at v2 — "Void & Ember") + root `CLAUDE.md`, plus the as-built state: chapters 00–04 shipped in `src/features/home/sections/*`.
2. Draft `PLAN.md`: file tree, chapter order (`00→06`) with per-chapter GSAP/Lenis technique, marking built chapters revise-vs-skip; true dependency delta (motion/form stack already installed); PRD→constants mapping (data layer already exists in `features/home/data/*`).
