...
| 6   | **Awards & Misc** _(the light-invert section)_ | ~66–74s      | A light-background list of 5 recognitions: GSAP "Site of the week", Awwwards "Honorable Mention", YouTube "Featured on Codegrid", Awwwards "Portfolio Honors nomination", landing.love "Best animations"                                                                           | Confirms the invert-section (§3.1b) is specifically an **awards/recognition** moment — matched directly as of 2026-08-25 — Sufyan's §3.5 awards left Journey for their own **09 Achievements** table |
...
> **v2.2 site-wide stack amendment (owner decision 2026-07-20 — supersedes §4.1–4.3 face columns below):** the hero pairing is promoted to the whole site as a **3-family stack**: **Instrument Serif** replaces Fraunces on `--font-display` (+ stays `--font-display-tail`; roman 400 via `@fontsource/instrument-serif`, italic 400 stays the self-hosted latin subset — the face ships 400 only, so the old Fraunces 300 uses now render 400); **Switzer** replaces General Sans on `--font-sans` (+ stays `--font-display-lead`; upgraded from the Medium static to `Switzer-Variable.woff2` 100–900, self-hosted); **Inter** replaces JetBrains Mono on `--font-mono` (`@fontsource-variable/inter`; the utility name is kept, labels are no longer monospace). Fraunces, General Sans, and JetBrains Mono are retired and uninstalled. The tables below keep the retired faces for historical rationale — read every "Fraunces" as Instrument Serif, "General Sans" as Switzer, "JetBrains Mono" as Inter.
...
- _Invert placement decided (PLAN v3.1 decision 6):_ the light-invert section is **08 Contact** (what the reference actually does). A second light surface joined it 2026-08-25 — 09 Achievements' filled rows, which is the reference's own awards treatment.

### 11.5 Journey — `05 — THE PATH`

- Merge Experience + Education into one vertical timeline (**Awards** §3.5 left for 09 Achievements, 2026-08-25); each entry (role/title, org, dates in Mono, one-line summary) reveals on enter. Chronological, most recent first.
...
- Awards moved to **09 Achievements** (2026-08-25): a flat table whose rows fill with a light bar wiping in from the left, one `mix-blend-difference` layer over neutral text. The micro-echo became the real thing.
