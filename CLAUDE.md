...
- **Content source**: `.agents/context/product_requirements.md` is the _only_ content source (profile, skills, tools, work, education, awards, projects, contact) — content is transcribed from it, never invented; unknown fields are omitted, not fabricated. The full dataset is already transcribed (21 skills, 6 journey items, 3 achievements, 6 projects, 3 contact channels) — reuse the data layer, never re-transcribe.
...
- **Fonts** (site-wide 3-family stack, owner decision 2026-07-20 — supersedes the older Fraunces/General Sans/JetBrains stack): **Instrument Serif** (display, roman via `@fontsource/instrument-serif`, italic self-hosted latin subset in `public/fonts/instrument-serif/`) · **Switzer** (body + hero lead, self-hosted variable woff2 in `public/fonts/switzer/`) · **Inter** (labels, `@fontsource-variable/inter` — the `font-mono` utility name is kept but renders Inter; labels are no longer monospace).
...
- **Chapters**: `00 Preloader · 01 Hero · 02 Manifesto · 03 About · 04 Project/Craft · 05 Journey · 06 Skills · 07 Gallery · 08 Contact · Footer` (PLAN v3.1 §0 — supersedes design_system §11's older 7-chapter order; 00–03 built). **Journey (05)** merges work experience + education into one timeline. Awards (§3.5) left it on 2026-08-25 and are their own chapter, **09 Achievements** (`achievements.data.ts`) — a flat recognition table matching the reference's own standalone awards section; don't fold them back in. A second owner-ask chapter, **10 Workflow** (`workflow.data.ts`), landed 2026-08-26 between Achievements and Articles — a pinned five-step process rail (Discovery → Planning → Development → Quality Testing → Maintenance). Like Articles it is **not PRD-transcribed** (the PRD has no process content at all); its copy is drafted against PRD §3.3 duties and the data file is the owner's only edit surface. Adding a chapter here means renumbering every eyebrow below it (Articles → 11, Contact → 12). Sections live in `src/features/home/sections/*`, content in `src/features/home/data/*` typed against `src/types/portfolio.ts`.
...
- `npx tsc --noEmit -p tsconfig.app.json` — typecheck only, no build. **The bare `npx tsc --noEmit` is a no-op** — the root `tsconfig.json` is solution-style (`files: []`), so it checks nothing (found 2026-08-25).
...
- Skills — two-kind architecture, both trees list the same **19** (every skill = one canonical + one stub): **KNOWLEDGE** (10, canonical `.agents/skills/`: gsap-lenis-motion, scrollytelling, animated-ui-references, tailwind-v4-shadcn, typescript-react-strict, tanstack-router, vite-setup, accessibility-reduced-motion, seo-meta, reference-capture); **PROCESS** (6, canonical `.claude/skills/`: plan-redesign, build-section, qa-audit, log-change, update-memory, discover-tooling); **VENDORED** (3, full impl only in `.claude/skills/`: impeccable, design-taste-frontend, cinematic-web — the latter two read through their repo-override notes/ADAPTATION.md).
