...
- **As built (revised 2026-07-13 to the reference-exact About beat, user request):** full-viewport composition (`min-h-svh`), still the veil's landing target (§4.6 grammar unchanged). Sans `aboutStatement` top-left with **Fraunces-italic focal phrases** (`aboutStatementEmphasis: ["software engineer", "precision and care"]` — new optional Profile field, the hero `taglineEmphasis` device generalized via a pure `emphasize()` splitter) → indented bio (`lg:ml-[20%] max-w-[36ch] text-paper`) → `MagneticButton` CV link in the reference's "INFO" spot → small mono `(3+)` left-gutter marker (from `profile.stats[0]`, aria-labelled) → **portrait panel filling the right 40vw**, rounded left corners `clamp(3rem,9vw,11rem)`, flush to the viewport's right edge and section bottom (mobile: stacked full-width `h-[55svh] rounded-t` block). **Stats count-up row removed by user decision** (data stays in `profile.stats`; the marker is its only surface). Logs: `2026-07-13-about.md` + `2026-07-13-about-reference-relayout.md`.
