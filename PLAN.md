...
- **Layout:** two-tier statement (`profile.aboutStatement` at `text-statement`, italic `font-display` emphasis device as in the hero tagline) → asymmetric grid: portrait (`ParallaxImage`, rounded-top per reference; ships on the `Image` fallback until a real photo is supplied) beside bio (`profile.bio`, `text-body`, ≤ 68ch) + stats row (3 · 7 · 10, mono values + muted labels) + CV link (`MagneticButton` → `profile.cvUrl`).
- **Motion:** `RevealText` lines for the statement; portrait clip reveal + scrub parallax (primitive defaults −8→8); stats **mono count-up** proxy tween (`power2.inOut`, dur 2 — the preloader-counter recipe), `once:true`. Reduced: static numbers, opacity-only.
- **Data:** exists (`profile.{aboutStatement,bio,stats,cvUrl,roles,location}`) — no delta.
