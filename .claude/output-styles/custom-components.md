...
| `h1`–`h6` | `<Heading level={2} variant="section">` | `level` 1–6 (default `1`) or `as="h2"`; `variant`: default \| display \| title \| subtitle \| section (default `"default"`; `display` = Fraunces `font-display text-display`). |
