...
> Twin: `.agents/rules/accessibility-performance.md` (portable mirror — same rule, keep in sync) + `.agents/skills/accessibility-reduced-motion` (deep-dive).
...
- Build the fallback alongside the effect, not after — all 8 shipped motion primitives (`RevealText` … `PathDraw`) already follow this contract; match it in any new one.
...
- Visible focus: accent ring `:focus-visible { outline: 2px solid var(--color-accent) }` (ember `#E8380F` per design_system v2 §3.3 — live in globals.css). Full keyboard nav; skip-to-content link.
...
- Overlays trap correctly: `SiteMenu` (z-80) and `Preloader` (z-90) use the React 19 `inert` pattern on background content (established in the Hero chapter QA round).
