...
description: Anti-slop frontend skill for landing pages, portfolios, and redesigns — brief inference, three design dials, real-design-system mapping, bias-correcting directives, AI-tell bans, and a strict pre-flight check. Use when designing, building, or reviewing any landing page, portfolio, or redesign so the output doesn't look templated.
---
...
> **In this repo:** apply with the project's overrides — Vite + GSAP (not Next + Motion), Fraunces is a deliberate, settled choice (the serif ban does not reopen it), tokens/primitives per design_system.md v2.

This SKILL.md is the router. Read the reference file for the stage you're at — in order for a fresh build, or jump straight to the relevant one:

| Stage / topic | Read |
| --- | --- |
| 0. Read the brief, output a one-line Design Read | `references/brief-inference.md` |
| 1. Set the three dials (VARIANCE / MOTION / DENSITY) + dial definitions | `references/dials.md` |
| 2. Map the brief to a real design system vs an aesthetic | `references/design-system-map.md` |
| 3. Default stack/state/icons/responsive conventions | `references/architecture-conventions.md` |
| 4. Design engineering directives — typography, color, layout, states, images, content density (the bias-correction core) | `references/design-directives.md` |
| 5. Context-aware proactivity — motion patterns + GSAP sticky-stack / horizontal-pan / reveal skeletons | `references/proactivity-motion.md` |
| 6. Performance & accessibility guardrails | `references/perf-a11y.md` |
| 8. Dark mode protocol | `references/dark-mode.md` |
| 9. AI tells — forbidden patterns | `references/ai-tells.md` |
| 10. Reference vocabulary (pattern names) | `references/vocabulary.md` |
| 11. Redesign protocol (audit-first) | `references/redesign-protocol.md` |
| 12. The block library contract | `references/block-library.md` |
| Appendices — real source-backed install commands + reference material | `references/appendices.md` |

Sections 13 (out of scope) and 14 (the final pre-flight check) stay inline below — the
pre-flight gate runs on every delivery.
