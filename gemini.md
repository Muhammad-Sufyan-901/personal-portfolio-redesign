...
- **Content source**: `.agents/context/product_requirements.md` is the *only* content source — transcribe, never invent; omit unknown fields. The dataset is already transcribed (21 skills, 6 journey items, 3 achievements, 6 projects, 3 contact channels) — reuse it, never re-transcribe. Persona: "Software Engineer · Web & Mobile"; stats 3 years · 7 stacks · 10 projects.
...
- **Skills — two-kind architecture** (both trees list the same 19; one canonical + one stub each): **KNOWLEDGE** skills are canonical in `.agents/skills/` (`gsap-lenis-motion`, `scrollytelling`, `animated-ui-references`, `tailwind-v4-shadcn`, `typescript-react-strict`, `tanstack-router`, `vite-setup`, `accessibility-reduced-motion`, `seo-meta`, `reference-capture`); **PROCESS** skills are canonical in `.claude/skills/` (`plan-redesign`, `build-section`, `qa-audit`, `log-change`, `update-memory`, `discover-tooling`); **VENDORED** toolkits live fully in `.claude/skills/` only (`impeccable`, `design-taste-frontend`, `cinematic-web` — read `cinematic-web/ADAPTATION.md` before using it here).
